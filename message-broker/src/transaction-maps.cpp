...
{
    shared_lock<shared_mutex> transaction_lock(transactions_mutex);
    return transactions[seqnum];
}

void TransactionMap::insertTransaction(sequence_t seqnum, const OutstandingTransaction& ta)
{
    unique_lock<shared_mutex> transaction_lock(transactions_mutex);
...
{
    shared_lock<shared_mutex> webapp_requests_lock(webapp_requests_mutex);
    return webapp_requests[seqnum];
}

void TransactionMap::insertWebappRequest(sequence_t seqnum, const WebappRequest& wa_rq)
{
    unique_lock<shared_mutex> webapp_requests_lock(webapp_requests_mutex);
...
{
    shared_lock<shared_mutex> b2b_origins_lock(b2b_origins_mutex);
    return b2b_origins[seqnum];
}

void TransactionMap::insertB2BOrigin(sequence_t seqnum, const string& origin)
{
    unique_lock<shared_mutex> b2b_origins_lock(b2b_origins_mutex);
